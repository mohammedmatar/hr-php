...
$menu_group=mysql_result($rs_select,0,'main');
}
...
<table border="0" width="37%" style="border-collapse: collapse; border: 1px solid #C0C0C0" dir="ltr">
	<tr>
		<td colspan="2" class="tdtitle"><b>lists settings</b> 
		</td>
	</tr>
	<tr>
	...
	<tr>
		<td width="35%" align="left" height="25"><span lang="en-us">
		<font size="2"><b>categorization</b></font></span></td>
		<td width="63%" height="25"><select size="1" name="menu" style="font-weight: 700">
		<option value="1" <?php if(!empty($_GET['id']) && $menu_group==1) echo "selected";?>>settings</option>
		<option value="2" <?php if(!empty($_GET['id']) && $menu_group==2) echo "selected";?>>employees information</option>
		<option value="3" <?php if(!empty($_GET['id']) && $menu_group==3) echo "selected";?>>reports</option>
		<option value="4" <?php if(!empty($_GET['id']) && $menu_group==4) echo "selected";?>>lists management</option>
		<option value="6" <?php if(!empty($_GET['id']) && $menu_group==5) echo "selected";?>>Statistical Reports</option>
		</select></td>
	</tr>
	<tr>
		<td width="35%" height="25" align="left"><span lang="en-us">
		<font size="2"><b>menu name</b></font></span></td>
		<td width="63%" height="25">
		<input type="text" name="name" size="36" value="<?php if(!empty($_GET["id"])) echo @mysql_result($rs_select,0,'name');?>"></td>
	</tr>
	<tr>
		<td width="35%" align="left"><b><font size="2">file link </font></b></td>
		<td width="63%">
		<input type="text" name="link_php" size="36" value="<?php if(!empty($_GET["id"])) echo @mysql_result($rs_select,0,'link');?>"></td>
	</tr>
	<tr>
		<td width="35%" align="left"><b><font size="2">status</font></b></td>
		<td width="63%">
...
if(!empty($_GET["id"]))		
 $pub=@mysql_result($rs_select,0,'publish');?>
		...
		<option value="1" <?php if(@$pub==1)echo "selected";?>>show</option>
		<option value="0" <?php if(@$pub==0)echo "selected";?>>hide</option></select></td>
...
/*
	...
	*/
	...
	while($row = mysql_fetch_row($result)){
		$table .="<tr><td align=\"center\">".$row[0]."</td><td align=\"center\"><font size=\"2\" face=\"Tahoma\">".$row[3]."</font></td><td align=\"center\" width=\"30\"><a href=\"".curPageURL().curPageParamter('id')."&id=".$row[0]."\"><img border=\"0\" alt=\"إختيار\" title=\"إختيار\"  src=\"images/icon-32-edit.jpg\" width=\"24\" height=\"25\"></a></td></tr>";	
	}
	$table .= "</table>";
	echo ''.$table.'';
