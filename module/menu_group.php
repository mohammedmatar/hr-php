...
//////////////////Command New////////////////////////////////
if(!empty($_POST["new"])){
header("location: index.php?mnu_id=4&page=menu_group.php");
}
...
//mysql_query("SET NAMES 'utf8'");
$rs_save=mysql_query("insert into menu_group (name,publish)values('$_POST[name]',$_POST[publish])");
if($rs_save)
$mess="data is saved";
else
...
$rs_save=mysql_query(" update menu_group set 
				name='$_POST[name]',
...
$rs_save=mysql_query("delete from menu_group where id=$_GET[id] ");
if($rs_save)
...
$rs_data=mysql_query("select *from menu_group ORDER BY id  DESC ");
$x=mysql_num_rows($rs_data);
$pub="3";
if(!empty($_GET["id"]) && empty($_POST["delete"])){
$rs_select=mysql_query("select *from menu_group where id=$_GET[id]");
}
...
<table border="0" width="37%" style="border-collapse: collapse; border: 1px solid #C0C0C0" dir="ltr">
	<tr>
		<td colspan="2" class="tdtitle"> <span lang="en-us">lists branches 
		settings </span></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<font size="2"><b>list name</b></font></span></td>
...
/*
	...
	mysql_query("SET NAMES 'utf8'");
	*/
	$result = mysql_query("select count(*) as noOfrows from menu_group");
	...
	$result = mysql_query("select * from menu_group order by id asc $testPage->limit");
