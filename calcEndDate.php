...
if($_SESSION["login"]!=1){
 ...
 include("config.php");
			...
			while ($row = mysql_fetch_row($result)) {
				...
				if($datediff['years'] % 2 == 1)
					$exi = $datediff['years'] - 1;
				else
					$exi = $datediff['years'];
				$exi += 2;	
				$dateOneYearAdded = strtotime(date("Y-m-d", strtotime($row[4])) . " +".$exi." year");	
				$endDate = date('Y-m-d', $dateOneYearAdded);
				$query = "update tb_employee set end_date ='".$endDate."' where id = ".$row[5];
		...
		echo '<hr /> Succ = '.$counter1.' Failed = '.$counter2.'<hr /> ';
