...
$sal1=$day_sal*$expir_day;
	...
	$sum_sal=@$sum_sal+round($sal1,2);//≈Ã„«·Ì «·„— » «·√”«”Ì

	if ($status==1){
	$cash_statas=(20 *$sal1)/100;
	$sum_cashstatus=@$sum_cashstatus + round($cash_statas,2);//≈Ã„«·Ì »œ· «·”ﬂ‰
}
	elseif($status==2){
...
$cash_chaild=0;
			...
			$sum_chaild=@$cash_chaild+round(@$sum_chaild,2);//≈Ã„«·Ì »œ· «·«»‰«¡
}
			if($chaild_count ==2){
	...
	$work=@mysql_result($rs_allaw,0,'fl_work');//ÿ»Ì⁄… ⁄„·
	if($work==1){
	$work=7*$sal1/100;
		$sum_work=@$sum_work+round($work,2);//≈Ã„«·Ì ÿ»Ì⁄… «·⁄„·
}
	else
	{
	$work=0;
...
/**********************«·»œ·« *************************/
	$cloth=@mysql_result($rs_allaw,0,'fl_cloth');//»œ· ·»”

if(@mysql_result($rs_allaw,0,'fl_cloth')!=0 || @mysql_result($rs_allaw,0,'fl_cloth')!="" ){
	$sum_close=@$sum_close+round($cloth,2);//≈Ã„«·Ì »œ· ·»”
}
	$manager=@mysql_result($rs_allaw,0,'fl_manager');//»œ· ≈œ«—…
	
if(@mysql_result($rs_allaw,0,'fl_manager')!=0 || @mysql_result($rs_allaw,0,'fl_manager')!="" ){
	$sum_manager=@$sum_manager+round($manager,2);//≈Ã„«·Ì »œ· ≈œ«—…
}
	$cashequal=@mysql_result($rs_allaw,0,'fl_cashequal');//„ﬂ«›∆Â
	
if(@mysql_result($rs_allaw,0,'fl_cashequal')!=0 || @mysql_result($rs_allaw,0,'fl_cashequal')!="" ){
	$sum_cashequal=@$sum_cashequal+round($cashequal,2);//≈Ã„«·Ì «·„ﬂ«›∆…
	}
	$gravity=@mysql_result($rs_allaw,0,'fl_gravity');//ŒÿÊ—…
	
if(@mysql_result($rs_allaw,0,'fl_gravity')!=0 || @mysql_result($rs_allaw,0,'fl_gravity')!="" ){
	$sum_gravity=@$sum_gravity+round($gravity,2);//≈Ã„«·Ì ŒÿÊ—…
}
	$mobil=@mysql_result($rs_allaw,0,'fl_mobil');//„Ê»«Ì·

if(@mysql_result($rs_allaw,0,'fl_mobil')!=0 || @mysql_result($rs_allaw,0,'fl_mobil')!="" ){
	
	$sum_mobile=@$sum_mobile+round($mobil,2);//≈Ã„«·Ì „Ê»«Ì·
	}
	$tension=@mysql_result($rs_allaw,0,'fl_tension');// Ê —
	
if(@mysql_result($rs_allaw,0,'fl_tension')!=0 || @mysql_result($rs_allaw,0,'fl_tension')!="" ){
	$sum_tension=@$sum_tension+round($tension,2);//≈Ã„«·Ì  Ê —
}
	$capital1=@mysql_result($rs_allaw,0,'fl_capital1');//⁄«’„…

	if(@mysql_result($rs_allaw,0,'fl_capital1')!=0 || @mysql_result($rs_allaw,0,'fl_capital1')!="" ){

	$sum_capital1=round($capital1,2)+@$sum_capital1;//Ã„·… ⁄«’„…
}
	$sal_differ=@mysql_result($rs_allaw,0,'fl_sal_differ');//›—ﬁ —« »

	if($sal_differ !=0 || $sal_differ !="" ){
	$sum_sal_differ=round($sal_differ,2)+@$sum_sal_differ;//Ã„·… ›—ﬁ —« »
	//echo $sal_differ;
	}
	
	$add20=@mysql_result(@$rs_allaw,0,'add20'); //«·“Ì«œ…
	
	if(@mysql_result(@$rs_allaw,0,'add20')!=0 || @mysql_result(@$rs_allaw,0,'add20')=="" ){
	$sum_sal_add20=round($add20,2)+@$sum_sal_add20;//Ã„·… «·“Ì«œ…
	}
...
/**********************≈Ã„«·Ì «·„— »*************************/
		$totsal=round(@$cloth,2)+round(@$manager,2)+round(@$cashequal,2)+round(@$gravity,2)+round(@$mobil,2)+round(@$tension,2)+round(@$capital1,2)+round(@$sal_differ,2)+round(@$work,2)+round(@$cash_chaild,2)+round(@$cash_statas,2)+round(@$sal1,2)+round(@$add20,2);//Ã„·… «·—« »
//		echo $i."<br>";
		//if(@$totsal != "" || @$totsal!=0)
	$sum_totsal=	round(@$sum_sal,2)+round(@$sum_chaild,2)+round(@$sum_cashstatus,2)+round(@$sum_work,2)+round(@$sum_mobile,2)+@$sum_manager+round(@$sum_gravity,2)+round(@$sum_close,2)+round(@$sum_capital1,2)+round(@$sum_tension,2)+round(@$sum_cashequal,2)+round(@$sum_sal_differ,2)+round(@$sum_sal_add20,2);
		//$sum_totsal=$totsal+$sum_totsal;//Ã„·… ≈Ã„«·Ì «·„— »« 
//	echo $sum_totsal."<br>";
/**********************‰Â«Ì…*************************/
/**********************«·≈” ﬁÿ«⁄« *************************/
	/*--------------------”·›Ì«  «·Ã„⁄Ì… «·ŒÌ—Ì…-----------------------*/
...
$rs_cut=mysql_query("select *from tb_tax where id=$emp_id");
	...
	$charity=@mysql_result($rs_cut,0,'charity');//Œ’„ «·Ã„⁄Ì… «·ŒÌ—Ì…
	$charity=$charity+$total_lone1;
	$sum_charity=@$sum_charity+$charity;

/*------------------------------------------*/
	$tax=@mysql_result($rs_cut,0,'tax');//«·÷—Ì»…
	$sum_tax=@$sum_tax+$tax;
...
$cut_daycash=$cut_daycash+($sal_day *@mysql_result($rs_sanitem,0,'discount_day')) ;//Õ”«» Œ’„ «·Ã“«¡« 
}
