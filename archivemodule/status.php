...
$rs_save=mysql_query("insert into   lk_status (name)values('$_POST[name]')");
if($rs_save){
...
$rs_save=mysql_query(" update   lk_status set 
...
$rs_save=mysql_query("delete from   lk_status where id=$_GET[id] ");
if($rs_save)
...
$rs_data=mysql_query("select *from   lk_status order by id desc  limit $_GET[pagging],$limit_lk");
$x=mysql_num_rows($rs_data);
$pub="3";
if(!empty($_GET["id"]) && empty($_POST["delete"])){
$rs_select=mysql_query("select *from   lk_status where id=$_GET[id]");
}
//////////////////Command New////////////////////////////////
if(!empty($_POST["new"])){
header("location: index.php?mnu_id=1&page=status.php ");
}
...
<table border="0" width="40%" style="border-collapse: collapse; border: 1px solid #C0C0C0" dir="rtl" class="tb_bgcolrform">
	<tr>
		<td colspan="2" class="tdtitle">الحالة الإجتماعية</td>
	</tr>
	<tr>
	...
	<tr>
		<td width="15%" height="25" align="left"><font size="2"><b>الحالة</b></font></td>
...
<?php 
	...
	mysql_query("SET NAMES 'utf8'");*/
	$result = mysql_query("select count(*) as noOfrows from lk_status");
	...
	$result = mysql_query("select * from lk_status order by id asc $testPage->limit");
