...
$rs_save=mysql_query("insert into   lk_Depart (name)values('$_POST[name]')");
if($rs_save){
...
$rs_save=mysql_query(" update   lk_Depart set 
...
$rs_save=mysql_query("delete from   lk_Depart where id=$_GET[id] ");
if($rs_save)
...
$rs_data=mysql_query("select *from   lk_Depart order by id desc  limit $_GET[pagging],$limit_lk");
$x=mysql_num_rows($rs_data);
$pub="3";
if(!empty($_GET["id"]) && empty($_POST["delete"])){
$rs_select=mysql_query("select *from   lk_Depart where id=$_GET[id]");
}
//////////////////Command New////////////////////////////////
if(!empty($_POST["new"])){
header("location: index.php?mnu_id=1&page=Depart.php ");
}
...
<table border="0" width="40%" style="border-collapse: collapse; border: 1px solid #C0C0C0" dir="ltr" class="tb_bgcolrform">
	...
	<tr>
		...
		<font size="2"><b>Department Name</b></font></span><font size="2"><b> </b></font></td>
...
<?php 
	...
	mysql_query("SET NAMES 'utf8'");*/
	$result = mysql_query("select count(*) as noOfrows from lk_Depart");
	...
	$result = mysql_query("select * from lk_Depart order by id asc $testPage->limit");
