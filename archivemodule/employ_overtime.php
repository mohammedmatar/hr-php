...
if(!empty($_POST["save"]) && empty($_GET["id"]) ){
	$rs_save=mysql_query("insert into  tb_overtime(emp_id,year,month,h1,h2,h3)values($_GET[emp_id],$_POST[year],$_POST[month],$_POST[h1],$_POST[h2],$_POST[h3])");
	if($rs_save){
			$mess="The Data is Saved";
		$new_emp_id = mysql_query("SELECT LAST_INSERT_ID() as id");
		$lrow = mysql_fetch_row($new_emp_id);
		header("location: index.php?mnu_id=2&page=employ_overtime.php&emp_id=".$_GET[emp_id]."&mess=".$mess."&id=".$lrow[0]);
	}else
			$mess="The Data is Not Saved";
}
//--------------------------Command Update----------------------------
if(!empty($_POST["save"]) && !empty($_GET["id"]) ){
	$rs_save=mysql_query(" update tb_overtime  set 
				year=$_POST[year],
				month=$_POST[month],
				h1=$_POST[h1],
				h2=$_POST[h2],
...
if(!empty($_POST["delete"]) && !empty($_GET["id"]) ){
	$rs_save=mysql_query("delete from    tb_overtime   where id=$_GET[id] ");
	if($rs_save){
		$mess="successful Delete";
		header("location: index.php?mnu_id=2&page=employ_overtime.php&emp_id=".$_GET[emp_id]."&mess=".$mess);
	}else
		$mess="Delete Failure !";
}
...
if(!empty($_GET["id"]) && empty($_POST["delete"])){
	$rs_select=mysql_query("select * from    tb_overtime  where id=$_GET[id]");
}
//---------------------------Load Employee Data----------------------------------
...
if(!empty($_POST["search"])){
	header("location: index.php?mnu_id=2&page=emp_search_overtime.php");
}
//////////////////Command search////////////////////////////////
if(!empty($_POST["new"])){
...
<table border="0" width="70%" style="border-collapse: collapse; border: 1px solid #C0C0C0" dir="ltr" class="tb_bgcolrform">
	<!--tr>
		<td colspan="6" class="tdtitleemp">الأجر الإضافي</td>
	</tr-->
	<tr>
	...
	<tr>
		...
		<font size="2"><b>Month</b></font></span></td>
...
<?php if(!empty($_GET["id"])) $monthselect= @mysql_result($rs_select,0,'month');else $monthselect=date("m")-1;?>	
	...
	<select size="1" name="month" style="font-weight: 700">
		<option value="01" <?php if(($monthselect==1 ) || (date('m')=='01' && (empty($_GET["id"])))) echo "selected";?>></option>
		<option value="02" <?php if(($monthselect==2 ) || (date('m')=='02' && (empty($_GET["id"])))) echo "selected";?>>Jan</option>
		<option value="03" <?php if(($monthselect==3 ) || (date('m')=='03' && (empty($_GET["id"])))) echo "selected";?>>Feb</option>
		...
		<td width="16%" height="25" align="left"><span lang="en-us">
		<font size="2"><b>Year</b></font></span></td>
	...
	<tr>
		<td width="17%" height="25" align="left"><span lang="en-us">Official 
		Hour</span></td>
	...
	<tr>	
		<td width="17%" height="25" align="left"><span lang="en-us">Vacation 
		Hours</span></td>
	...
	<tr>	
		<td width="17%" height="25" align="left"><span lang="en-us">Off Days 
		Hours</span></td>
...
<?php 
	...
	$result = mysql_query("select count(*) as noOfrows from tb_overtime where emp_id=$_GET[emp_id] ");
	...
	$result = mysql_query("select id,year,month,h1,h2,h3 from tb_overtime  where emp_id=$_GET[emp_id]  order by year desc,month desc $testPage->limit");
